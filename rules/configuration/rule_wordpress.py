...
class Rule:
  def __init__(self):
    ...
    self.rule_description = 'Wordpress Misconfiguration'
    self.rule_confirm = 'Remote Server Wordpress is Misconfigured'
    self.rule_details = ''
    self.rule_mitigation = '''Wordpress may have been misconfigured and may had leaked application data.
      ...
      if resp is not None:
        for match in values['match']:
          if match in resp.text:
            self.rule_details = 'Wordpress Misconfiguration - {} at {}'.format(app_title, uri)
